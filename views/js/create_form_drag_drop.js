...
function makeBold() {
	if (selectedTextBox) {
		...
		if (selectedText) {
			...
			// Check if the selected text is within the selectedTextBox
			if (selectedTextBox.contains(parentElement)) {
				// Check if the selected text is already bold
				const isAlreadyBold = parentElement.classList.contains("w3-bold");

				// Toggle the bold class
				if (isAlreadyBold) {
					parentElement.classList.remove('w3-bold');
				} else {
					parentElement.classList.add('w3-bold');
				}
			}
		}
	}
